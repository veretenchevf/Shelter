// Посланиесебе самому: Больше так никогда не делай! я провесь проект.
// Это самый настоящий адовый говнокод ever.
let log = console.log;
const catalogue = [
    {
      "name": "Jennifer",
...
let randValue = newCatalogue.splice(0, Math.floor(Math.random() * 8));
...
document.addEventListener("DOMContentLoaded", () => {
    newCatalogue.forEach(function(pet){
...
PAG_BTNS.addEventListener('click', () => {
    ...
    if(page == maxPage){
        ...
        // log(parseInt(getComputedStyle(CATALOGUE).top))
    ...
    // if(CATALOGUE.style.top == catalogueBottom){
    //     BTN_START.removeAttribute('disabled');
    //     BTN_PREV.removeAttribute('disabled');
    //     BTN_NEXT.setAttribute('disabled', 'true');
    //     BTN_END.setAttribute('disabled', 'true');
    // }else if(parseInt(getComputedStyle(CATALOGUE).top) !== 0){
    //     BTN_START.removeAttribute('disabled');
    //     BTN_PREV.removeAttribute('disabled');
    //     BTN_NEXT.removeAttribute('disabled');
    //     BTN_END.removeAttribute('disabled');
...
BTN_PREV.addEventListener('click', (e) => {
    if(parseInt(getComputedStyle(CATALOGUE).top) === 0){
        BTN_PREV.setAttribute('disabled', 'true')
    }else{CATALOGUE.style.top = `${parseInt(getComputedStyle(CATALOGUE).top) + 930}px`;
    page -= 1}
})
